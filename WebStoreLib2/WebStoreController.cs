...
namespace WebStoreLib
{
    ...
    public class WebStoreController
    {
        ...
        private static CategoryFactory CategoryFactory = FactoryHost.CategoryFactory;

        private static ProductFactory ProductFactory = FactoryHost.ProductFactory;

        private static StockAvailableFactory StockAvailableFactory = FactoryHost.StockAvailableFactory;

        private static ImageFactory ImageFactory = FactoryHost.ImageFactory;

        private static language RusLang = FactoryHost.RusLang;

        //private List<category> StoreCategories;
        //private List<product> StoreProducts;

        private static List<category> _storeCategories;
        private static List<product> _storeProducts;

        private static List<category> StoreCategories
        {
            get
            {
                if (_storeCategories == null)
                {
                    _storeCategories = CategoryFactory.GetAll();
                }
        ...
        private static List<product> StoreProducts
        {
            get
            {
                if (_storeProducts == null)
                {
                    _storeProducts = ProductFactory.GetAll();
                }
        ...
        private bool SetLinkRewrite(object store, object shop)
        {
            ...
            string linkRewrite = shopDuck.LinkRewrite;
            string title = shopDuck.Title;
        ...
        private bool SetParentCategory(category store, AdvCategory advCategory)
        {
            bool needSave = true;

            int rootParentId = 2;

            int? newParentId = advCategory.Parent == null ? rootParentId : advCategory.Parent.Number;
        ...
        private bool SetCategory(product prod, Adv24au shopProduct)
        {
            Sync(shopProduct.Category);

            bool needSave = true;

            if (prod.associations.categories.Count > 0)
            {
                needSave = prod.associations.categories[0].id != shopProduct.Category.Number.Value;
            }
            ...
            if (needSave)
            {
                ...
                prod.associations.categories[0].id = shopProduct.Category.Number.Value;
                prod.id_category_default = shopProduct.Category.Number;
            }
        ...
        public void Sync(AdvCategory shopCategory)
        {
            if (shopCategory.Parent != null)
            {
                Sync(shopCategory.Parent);
            }

            var cat = StoreCategories.FirstOrDefault(sc => sc.id == shopCategory.Number);
            bool isNew = cat == null;

            bool needSave = false;


            //if (!shopCategory.Published && !isNew)
            //{
            //    cat.active = 0;
            //}
            // else
            ...
            shopCategory.Published = true;

            if (isNew)
            {
                cat = CategoryFactory.Add(cat);
                shopCategory.Number = (int)cat.id;
            ...
            if (shopCategory.Published)
            {
                SyncCatImages(shopCategory, cat);
            }

            //foreach (var shopProduct in shopCategory.Advs)
            //{
            //    Sync(shopProduct);
            //}
        ...
        public void Sync(Adv24au advProduct)
        {
            var prod = StoreProducts.FirstOrDefault(sc => sc.id == advProduct.Number);
            bool isNew = prod == null;

            bool needSave = false;

            //if (!advProduct.Published && !isNew)
            //{
            //    prod.active = 0;
            //}
            //else
            {
                ...
                needSave |= Set(prod.name, advProduct.Title);
                needSave |= Set(prod.description_short, string.Empty);

                if (advProduct.Description != null)
                    advProduct.Description = advProduct.Description.Replace("\r", "").Replace("\n", "");

                needSave |= Set(prod.description, advProduct.Description);

                needSave |= SetLinkRewrite(prod, advProduct);
                needSave |= SetCategory(prod, advProduct);


                needSave |= Set(prod, p => p.price, advProduct.Price);

                needSave |= Set(prod, p => p.on_sale, advProduct.IsSale ? 1 : 0);
                needSave |= Set(prod, p => p.unity, "шт");// advProduct.Unity);
                ...
                advProduct.Published = true;

                if (isNew)
                {
                    prod = ProductFactory.Add(prod);
                    advProduct.Number = (int)prod.id;

                    operationName = "добавлен";
                    Messenger.Write("В online {0} товар: {1}", operationName, advProduct.Title);
                ...
                else
                {
                    ...
                    {
                        Messenger.Write("Товар - {0}, не изменен", advProduct.Title);
                    }
                }
                ...
                // if (isNew)
                {
                    ...
                    stockAvailable.quantity = 10; // пока просто, чтобы можно было заказывать // shopProduct.Quantity ?? 0;                    
                    StockAvailableFactory.Update(stockAvailable);
                }


                if (isNew || needSave)
                    SyncProdImages(advProduct, prod);
            }
        }
        ...
        private void SyncCatImages(AdvCategory shopCat, category storeCat)
        {
        ...
        //private void SyncProdImages(Adv24au shopProd, product storeProd)
        //{
        ...
        private void SyncProdImages(Adv24au advProd, product storeProd)
        {
            if (advProd.Product.GetImagePath().IsEmpty()) return;

            var storeImages = storeProd.associations.images;

            if (storeImages.Any())
            {
                long delId = storeImages.First().id;
                ImageFactory.DeleteProductImage((long)storeProd.id, delId);
            }

            string imagePath = advProd.Product.GetImagePath();
            ImageFactory.AddProductImage(storeProd.id.Value, imagePath);

            storeImages = ProductFactory.Get(storeProd.id.Value).associations.images;

            storeProd.id_default_image = storeImages.First().id;
            ProductFactory.Update(storeProd);

            advProd.Image = storeProd.id_default_image.ToString();
        ...
        */
        ...
        public static void SetNumbersByIdsInReferences(params Adv24au[] advs)
        {
            ...
            var filterDict = new Dictionary<string, string> { { "reference", "[" + sb.ToString() + "]" } };
            var products = ProductFactory.GetByFilter(filterDict, null, null);

            var resultDict = new Dictionary<int, int>();

            foreach (var product in products)
            {
                var reference = Convert.ToInt32(product.reference);
                var id = Convert.ToInt32(product.id);
                resultDict.Add(reference, id);
            }

            foreach (var adv in advs)
            {
                adv.Number = resultDict[adv.Id];
            }
        }
    }
